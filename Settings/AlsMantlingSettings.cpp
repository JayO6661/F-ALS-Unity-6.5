...
{
	if (ChangedEvent.GetPropertyName() == GET_MEMBER_NAME_ANSI_STRING_VIEW_CHECKED(FAlsGeneralMantlingSettings, SlopeAngleThreshold))
	{
		SlopeAngleThresholdCos = FMath::Cos(FMath::DegreesToRadians(SlopeAngleThreshold));
	}
	else if (ChangedEvent.GetPropertyName() ==
	         GET_MEMBER_NAME_ANSI_STRING_VIEW_CHECKED(FAlsGeneralMantlingSettings, MantlingTraceResponseChannels))
	{
